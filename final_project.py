...
class Hero:
    def __init__(self,name="", id=0):
        ...
        self.id = id
        self.wl_list = {}
        read_file = open("hero_name.txt", "r")
...
####################################################################################
    ...
    button2 = tkinter.Button(gui, text='load from OpenDotaAPI', width=20, command=lambda: loadMatchups(Hero_list,"get_matchups.txt"))
    ...
    #get matchups from api fresh
    # updateMatchups(Hero_list)
    # loadMatchups(Hero_list,"get_matchups.txt")

    #load from old matchups
    #loadMatchups(Hero_list)


    #get edges to counter heroes
    # for id in Hero_list:
    #     Hero_list[id].edgeUpdate()

    #get new match data from api
    # getMatchesOpenDota(Hero_list)
    # getMatchesSteam(Hero_list)


    #save modified matchups
    # saveMatchups(Hero_list)
...
def getProMatchesSteam(Hero_list):

    file = open("./final_project/proPlayers.json")
    response_data = json.load(file)
    for player in response_data:
        account_id = player["account_id"]
        match_response = requests.get("https://api.steampowered.com/IDOTA2Match_570/GetMatchHistory/v1/?key=CFCC44456136B4AE0AAF4E97E89CB551&skill=3&account_id="+str(account_id))
        matches = match_response.json()["result"]["matches"]
    ...
    for id in Hero_list:
        Hero_list[id].edgeUpdate()
    text.insert("end",  "Steam pro matches data loaded\n")
    file.close()
...
def updateMatchups(Hero_list):
    if os.path.exists("get_matchups.txt"):
        os.remove("get_matchups.txt")
    read_file = open("get_matchups.txt", "w")
...
def saveMatchups(Hero_list):
    if os.path.exists("hero_matchups.txt"):
        os.remove("hero_matchups.txt")
    read_file = open("hero_matchups.txt", "w")
    for num in Hero_list:
        string=str(num)+"%"
        list=[]
...
def loadMatchups(Hero_list, file="hero_matchups.txt"):
...
#steam ?key=CFCC44456136B4AE0AAF4E97E89CB551
#https://api.steampowered.com/IDOTA2Match_570/GetMatchHistory/v1/?key=CFCC44456136B4AE0AAF4E97E89CB551&game_mode=1&skill=3
        # match_response = requests.get("https://api.steampowered.com/IDOTA2Match_570/GetMatchDetails/v1/?key=CFCC44456136B4AE0AAF4E97E89CB551&match_id="+str(match_id))
        # if match_response.status_code==200 and "players" in match_response.json()["result"]:
        #     lobby_type = match_response.json()["result"]["lobby_type"]
        #     human_players = match_response.json()["result"]["human_players"]
        #     game_mode = match_response.json()["result"]["game_mode"]
        #     # print(type(lobby_type))
        #     # print(lobby_type)
        #     # print(type(human_players))
        #     # print(human_players)
        #     if (lobby_type==7 or lobby_type==12) and human_players==10 and (game_mode==22 or game_mode==3) and "radiant_win" in match_response.json()["result"]:
        #         success +=1
        #         radiant_win = match_response.json()["result"]["radiant_win"]
        #         radiant_team, dire_team = [], []
        #         for player in match_response.json()["result"]["players"]:
        #             if player["team_number"] == 0:
        #                 radiant_team.append(player["hero_id"])
        #             else:
        #                 dire_team.append(player["hero_id"])
